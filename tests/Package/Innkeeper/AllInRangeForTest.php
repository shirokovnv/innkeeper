...
/**
 * @covers \Shirokovnv\Innkeeper\Innkeeper::allInRangeFor
 */
class AllInRangeForTest extends InnkeeperTestCase
{
    /**
     * @return void
     */
    public function testEmptyCollection(): void
    {
        ...
        $bookings = $innkeeper->allInRangeFor($room, $initial_date->subYear(), $initial_date->addYear());
    ...
    public function testNonEmptyCollection(): void
    {
        ...
        $bookings = $innkeeper->allInRangeFor($room, $initial_date, $initial_date->addHours($count_bookings));
        ...
        $bookings = $innkeeper->allInRangeFor($room, $initial_date, $initial_date->addHour());
        $this->assertEquals(1, $bookings->count());

        // Test query bookings in before interval
        $bookings = $innkeeper->allInRangeFor($room, $initial_date->subDay(), $initial_date);
        $this->assertEquals(0, $bookings->count());

        // Test query bookings in after interval
        $bookings = $innkeeper->allInRangeFor($room, $initial_date->addDay(), $initial_date->addDays(2));
        $this->assertEquals(0, $bookings->count());
    }
}
